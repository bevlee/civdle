...
<figure aria-label={`${topic} illustrated example`}>
  ...
  {:else if topic === "modes"}
    ...
    {:else}
      <div class="cards"><div><UnitCard unitId="mage" stars={3} size="sm" /><span>Starting pool · up to 3★</span></div><div><UnitCard unitId="swordsman" stars={4} size="sm" /><span>War Forge · unlock 4★</span></div><div><UnitCard unitId="champion" stars={5} size="sm" /><span>Master Forge · unlock 5★</span></div></div>
    {/if}
